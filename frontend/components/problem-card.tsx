  ...
  return (
    <Card className="h-full flex flex-col">
      <Link href={`/problems/${problem.id}`} className="flex-grow">
        <CardHeader>
          <CardTitle className="text-xl text-balance">{problem.title}</CardTitle>
        ...
        <CardContent className="space-y-4">
          <p className="text-sm text-muted-foreground line-clamp-2 leading-relaxed">{problem.content}</p>
          {problem.answer && (
        ...
        <AlertDialog>
          ...
          <AlertDialogContent onClick={(e) => e.stopPropagation()}>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                この操作は元に戻せません。問題「{problem.title}」を完全に削除します。
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
