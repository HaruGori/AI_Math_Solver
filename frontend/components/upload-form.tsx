...
import { problemsApi, uploadApi, tagsApi } from "@/lib/api"
...
export function UploadForm() {
  ...
  const [uploadedProblemId, setUploadedProblemId] = useState<number | null>(null); // New state for problem ID
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <Tabs defaultValue="image" className="w-full" onValueChange={(v) => setActiveTab(v as "image" | "text")}>
        ...
        <TabsContent value="image" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">問題の画像をアップロード</CardTitle>
              <CardDescription>写真を撮るか、既存の画像をアップロードしてください</CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <div className="space-y-4">
          ...
          {uploadedProblemId && (
            <Button asChild className="w-full">
              <Link href={`/problems/${uploadedProblemId}`}>問題詳細を見る</Link>
            </Button>
          )}
        </div>
      )}
      ...
      <div className="flex gap-3">
        <Button type="button" variant="outline" className="flex-1 bg-transparent" onClick={() => router.push("/")}>
          キャンセル
        </Button>
        <Button type="submit" className="flex-1 gap-2" disabled={isSubmitting || !title}>
          {isSubmitting ? (
            <>処理中...</>
          ) : (
            <>
